...
//{
//    columns[Amount] = MyntaUnits::getAmountColumnTitle(walletModel->getOptionsModel()->getDisplayUnit());
...
QString TransactionTableModel::formatTxAmount(const TransactionRecord *wtx, bool showUnconfirmed, MyntaUnits::SeparatorStyle separators) const
{
    QString str;
    switch(wtx->type) {
        case TransactionRecord::Issue:
        ...
        default:
            {
            str = MyntaUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), wtx->credit + wtx->debit,
                                             false, separators);
            } break;
    }
    if (showUnconfirmed) {
...
QVariant TransactionTableModel::data(const QModelIndex &index, int role) const
{
    ...
    case Qt::DisplayRole:
        ...
        case Amount:
            return formatTxAmount(rec, true, MyntaUnits::separatorAlways);
        case AssetName:
            if (rec->assetName != "MYNTA")
               return QString::fromStdString(rec->assetName);
            else
               return QString(MyntaUnits::name(walletModel->getOptionsModel()->getDisplayUnit()));
        } // no default case, so the compiler can warn about missing cases
        assert(false);
    case Qt::EditRole:
    ...
    case TxPlainTextRole:
        {
            ...
            details.append(formatTxAmount(rec, false, MyntaUnits::separatorNever));
            return details;
        }
    case ConfirmedRole:
    ...
    case FormattedAmountRole:
        ...
        return formatTxAmount(rec, false, MyntaUnits::separatorNever);
    case AssetNameRole:
        {
            QString assetName;
            if (rec->assetName != "MYNTA")
               ...
               assetName.append(QString(MyntaUnits::name(walletModel->getOptionsModel()->getDisplayUnit())));
            return assetName;
        }
    case StatusRole:
...
QVariant TransactionTableModel::headerData(int section, Qt::Orientation orientation, int role) const
{
    if(orientation == Qt::Horizontal)
    {
        ...
        } else if (role == Qt::ToolTipRole)
        {
            ...
            case AssetName:
                return tr("The asset (or MYNTA) removed or added to balance.");
            }
        }
    }
    return QVariant();
