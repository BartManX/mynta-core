...
{
    GUIUtil::setClipboard(MyntaUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void AssetControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    int nDisplayUnit = MyntaUnits::MYNTA;
    ...
    l2->setText(MyntaUnits::formatWithCustomName(QString::fromStdString(strAssetName), nAssetAmount));        // Amount
    l3->setText(MyntaUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(MyntaUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));        // Bytes
    l7->setText(fDust ? tr("yes") : tr("no"));                               // Dust
    l8->setText(MyntaUnits::formatWithCustomName(QString::fromStdString(strAssetName), nChange));        // Change
    if (nPayFee > 0)
    {
...
void AssetControlDialog::updateView()
{
    ...
    for (const std::pair<QString, std::vector<COutput>> &coins : mapAssetCoins) {
        ...
        for (const COutput &out : coins.second) {
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, MyntaUnits::format(nDisplayUnit, nAmount));
        ...
        if (treeMode) {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, MyntaUnits::format(nDisplayUnit, nSum));
