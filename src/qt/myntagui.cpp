...
const std::string MyntaGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString MyntaGUI::DEFAULT_WALLET = "~Default";
...
std::array<CurrencyUnitDetails, 5> CurrencyUnits::CurrencyOptions = { {
    { "BTC",    "MYNTABTC"  , 1,          8},
    { "mBTC",   "MYNTABTC"  , 1000,       5},
    { "µBTC",   "MYNTABTC"  , 1000000,    2},
    { "Satoshi","MYNTABTC"  , 100000000,  0},
    { "USDT",   "MYNTAUSDT" , 1,          5}
} };

static bool ThreadSafeMessageBox(MyntaGUI *gui, const std::string& message, const std::string& caption, unsigned int style);

MyntaGUI::MyntaGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
...

{
    ...
    {
        progressBar->setStyleSheet("QProgressBar { background-color: #1e293b; border: 1px solid #334155; border-radius: 7px; padding: 1px; text-align: center; color: white; } QProgressBar::chunk { background: QLinearGradient(x1: 0, y1: 0, x2: 1, y2: 0, stop: 0 #f59e0b, stop: 1 #fbbf24); border-radius: 7px; margin: 0px; }");
    }
...
void MyntaGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Mynta address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and mynta: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to MYNTA addresses"));
...
//    messagingAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Mynta addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Mynta addresses"));
    ...
    openAction->setStatusTip(tr("Open a mynta: URI or payment request"));
...
//        m_toolbar->addAction(votingAction);
        ...
        labelCurrentMarket->setText(tr("Mynta Market Price"));
...
void MyntaGUI::updateIconsOnlyToolbar(bool IconsOnly)
{
    if(IconsOnly) {
        labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/myntatext")));
        m_toolbar->setMaximumWidth(65);
...
void MyntaGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool MyntaGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool MyntaGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void MyntaGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void MyntaGUI::setWalletActionsEnabled(bool enabled)
{
...
void MyntaGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void MyntaGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void MyntaGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void MyntaGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void MyntaGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void MyntaGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void MyntaGUI::openClicked()
{
...
void MyntaGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void MyntaGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void MyntaGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void MyntaGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void MyntaGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void MyntaGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** MYNTA START */
void MyntaGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void MyntaGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void MyntaGUI::gotoManageAssetsPage()
{
    manageAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoManageAssetsPage();
};

void MyntaGUI::gotoRestrictedAssetsPage()
{
...
void MyntaGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Mynta network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void MyntaGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void MyntaGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void MyntaGUI::updateHeadersSyncProgressLabel()
{
...
void MyntaGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void MyntaGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Mynta"); // default title
...
void MyntaGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void MyntaGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void MyntaGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "MYNTA")
        msg += tr("Amount: %1\n").arg(MyntaUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(MyntaUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void MyntaGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void MyntaGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool MyntaGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool MyntaGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void MyntaGUI::setHDStatus(int hdEnabled)
{
    QString icon = "";
...
void MyntaGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void MyntaGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void MyntaGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void MyntaGUI::detectShutdown()
{
...
void MyntaGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void MyntaGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void MyntaGUI::showModalOverlay()
{
...
static bool ThreadSafeMnemonic(MyntaGUI *gui, unsigned int style)
{
    bool modal = (style & CClientUIInterface::MODAL);
    // The SECURE flag has no effect in the Qt GUI.
    // bool secure = (style & CClientUIInterface::SECURE);
...
void MyntaGUI::subscribeToCoreSignals()
{
...
void MyntaGUI::unsubscribeFromCoreSignals()
{
...
void MyntaGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void MyntaGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<MyntaUnits::Unit> units = MyntaUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const MyntaUnits::Unit unit : units)
    {
    #ifndef QTversionPreFiveEleven
        max_width = qMax(max_width, fm.horizontalAdvance(MyntaUnits::name(unit)));
    #else
        max_width = qMax(max_width, fm.width(MyntaUnits::name(unit)));
    #endif
    }
...
{
    menu = new QMenu(this);
    for (MyntaUnits::Unit u : MyntaUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(MyntaUnits::name(u)), this);
...
{
    setText(MyntaUnits::name(newUnits));
}
...
void MyntaGUI::currencySelectionChanged(int unitIndex)
{
    if(clientModel && clientModel->getOptionsModel())
    {
...
void MyntaGUI::onCurrencyChange(int newIndex)
{
    qDebug() << "MyntaGUI::onPriceUnitChange: " + QString::number(newIndex);
