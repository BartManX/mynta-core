...
/**
 * Alias for GetParams() - provided for compatibility with code that uses
 * the shorter Params() naming convention (common in Bitcoin Core forks).
 */
inline const CChainParams& Params() { return GetParams(); }
 ...
 * 
 * Call after SelectParams() and periodically during runtime to ensure
 * the globals used by block hashing haven't been corrupted.
 * 
 * @return true if all globals match consensus params
 */
 ...
 * 
 * Calls ValidateConsensusGlobals() and aborts if validation fails.
 * Use in debug builds at critical consensus code paths.
 */
