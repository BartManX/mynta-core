...
UniValue generatetoaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "2. address      (string, required) The address to send the newly generated MYNTA to.\n"
    ...
    // One-time log explaining generatetoaddress behavior
    static bool fLoggedGenerateInfo = false;
    if (!fLoggedGenerateInfo) {
        LogPrintf("generatetoaddress RPC invoked:\n");
        LogPrintf("  Function: RPC-triggered block generation using standard consensus rules.\n");
        LogPrintf("  Effect: Mines blocks via internal miner; does not bypass network validation.\n");
        LogPrintf("  Consensus: All generated blocks meet full PoW and validation requirements.\n");
        LogPrintf("  Note: Distinct from -solomining, which only affects peer connection checks.\n");
        fLoggedGenerateInfo = true;
    }
...
UniValue getmininginfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"errors\": \"...\"            (string) DEPRECATED. Same as warnings. Only shown when myntad is started with -deprecatedrpc=getmininginfo\n"
            "}\n"
            "\nExamples:\n"
...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
    ...
    // MYNTA: Allow solo mining via -solomining flag for initial chain bootstrap
    bool fSoloMining = gArgs.GetBoolArg("-solomining", false);
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0 && 
        !fSoloMining &&
        !gArgs.GetBoolArg("-bypassdownload", false))
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Mynta is not connected!");

    // One-time log when getblocktemplate is used in solo mining mode
    static bool fLoggedSoloGBT = false;
    if (fSoloMining && g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0 && !fLoggedSoloGBT) {
        LogPrintf("getblocktemplate: Operating in -solomining mode (no peer connections).\n");
        LogPrintf("  Block templates are constructed locally without peer validation.\n");
        LogPrintf("  Consensus rules remain enforced; mined blocks must meet full PoW requirements.\n");
        fLoggedSoloGBT = true;
    }
...
UniValue estimatefee(const JSONRPCRequest& request)
{
    ...
    if (!IsDeprecatedRPCEnabled("estimatefee")) {
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "estimatefee is deprecated and will be fully removed in v0.17. "
            "To use estimatefee in v0.16, restart myntad with -deprecatedrpc=estimatefee.\n"
