...
UniValue gettxout(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of Mynta addresses\n"
...
UniValue getblockchaininfo(const JSONRPCRequest& request)
{
    ...
    // MYNTA LAUNCH: Show chain start time information
    if (consensusParams.nChainStartTime > 0) {
        int64_t nCurrentTime = GetTime();
        obj.push_back(Pair("chain_start_time", consensusParams.nChainStartTime));
        obj.push_back(Pair("chain_start_date", "January 14, 2026 4:00 PM PST"));
        if (nCurrentTime < consensusParams.nChainStartTime) {
            int64_t nSecondsUntilLaunch = consensusParams.nChainStartTime - nCurrentTime;
            int64_t nDays = nSecondsUntilLaunch / 86400;
            int64_t nHours = (nSecondsUntilLaunch % 86400) / 3600;
            int64_t nMins = (nSecondsUntilLaunch % 3600) / 60;
            int64_t nSecs = nSecondsUntilLaunch % 60;
            obj.push_back(Pair("chain_started", false));
            obj.push_back(Pair("seconds_until_launch", nSecondsUntilLaunch));
            obj.push_back(Pair("time_until_launch", strprintf("%dd %dh %dm %ds", nDays, nHours, nMins, nSecs)));
        } else {
            obj.push_back(Pair("chain_started", true));
        }
    }

    // ============================================================
    // CHAINLOCKS STATUS - Operator & Infrastructure Visibility
    // All values derived from deterministic chain state only.
    // This is purely informational and does NOT affect consensus.
    // ============================================================
    {
        UniValue chainlocks(UniValue::VOBJ);
        
        if (llmq::chainLocksManager) {
            bool enabled = llmq::chainLocksManager->IsChainLockActive();
            chainlocks.push_back(Pair("enabled", enabled));
            
            if (enabled) {
                // When enabled, show activation height only
                chainlocks.push_back(Pair("activation_height", 
                    llmq::chainLocksManager->GetActivationHeight()));
            } else {
                // When not enabled, show diagnostic information
                chainlocks.push_back(Pair("activation_height", UniValue()));  // null
                chainlocks.push_back(Pair("next_activation_attempt", 
                    llmq::chainLocksManager->GetNextActivationAttemptHeight()));
                chainlocks.push_back(Pair("required_masternodes", 
                    llmq::CHAINLOCK_REQUIRED_MASTERNODES));
                chainlocks.push_back(Pair("eligible_masternodes", 
                    llmq::chainLocksManager->GetEligibleMasternodeCount()));
                
                // Get the reason as an enumerated string (no free-text ambiguity)
                llmq::ChainLockActivationReason reason = 
                    llmq::chainLocksManager->GetActivationStatus();
                chainlocks.push_back(Pair("reason", 
                    llmq::ChainLockReasonToString(reason)));
            }
            
            // Current best ChainLock info
            int bestCLHeight = llmq::chainLocksManager->GetBestChainLockHeight();
            if (bestCLHeight > 0) {
                chainlocks.push_back(Pair("best_chainlock_height", bestCLHeight));
            }
        } else {
            // ChainLocks manager not initialized
            chainlocks.push_back(Pair("enabled", false));
            chainlocks.push_back(Pair("reason", "not_initialized"));
        }
        
        obj.push_back(Pair("chainlocks", chainlocks));
    }
