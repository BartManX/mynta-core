...
inline void UnserializeTransaction(TxType& tx, Stream& s) {
    ...
    // For special transactions (DIP2/DIP3), nType follows nVersion for version >= 3
    if (tx.nVersion >= 3) {
        s >> tx.nType;
    } else {
        tx.nType = 0;
    ...
    // For special transactions, read the extra payload after nLockTime
...
inline void SerializeTransaction(const TxType& tx, Stream& s) {
    ...
    // For special transactions, write the extra payload after nLockTime
    if (tx.nVersion >= 3 && tx.nType != 0) {
        s << tx.vExtraPayload;
    }
}
