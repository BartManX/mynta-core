 ...
 * between myntad, and mynta-qt and non-server tools.
*/
...
void PrepareShutdown()
{
    ...
    // ============================================================
    // LLMQ and PoSe Subsystem Shutdown
    // Stop in reverse order of initialization
    // ============================================================
    StopPoSe();
    llmq::StopChainLocks();
    llmq::StopInstantSend();
    llmq::StopLLMQ();
    LogPrintf("LLMQ and PoSe subsystems stopped\n");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-masternode", _("Enable masternode mode (requires -masternodeblsprivkey)"));
    strUsage += HelpMessageOpt("-masternodeblsprivkey=<key>", _("Set the masternode operator BLS private key (64 hex characters). This key must match a registered masternode's operator public key."));
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single Mynta process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    // Log -solomining mode if enabled
    if (gArgs.GetBoolArg("-solomining", false)) {
        LogPrintf("=== SOLO MINING MODE ENABLED ===\n");
        LogPrintf("  -solomining: Bypasses peer connection requirement for getblocktemplate RPC.\n");
        LogPrintf("  Effect: Allows block template construction without connected peers.\n");
        LogPrintf("  Use case: Controlled testing, isolated operation, or initial chain bootstrap.\n");
        LogPrintf("  Consensus: Block validity rules are UNCHANGED. All blocks must meet standard PoW.\n");
        LogPrintf("  Operator opt-in: This mode requires explicit -solomining flag.\n");
        LogPrintf("================================\n");
    }
    ...
    while (!fLoaded && !fRequestShutdown) {
        ...
        // Initialize Evolution database and masternode manager after block index is loaded
        if (fLoaded) {
            // Initialize evoDb if not already initialized
            if (!evoDb) {
                evoDb = std::make_unique<CEvoDB>(nBlockTreeDBCache, false, fReset);
                LogPrintf("Initialized Evolution database\n");
            }
            
            // Initialize deterministic masternode manager
            if (!deterministicMNManager) {
                deterministicMNManager = std::make_unique<CDeterministicMNManager>(*evoDb);
                if (!deterministicMNManager->Init()) {
                    return InitError(_("Failed to initialize masternode manager"));
                }
                LogPrintf("Initialized deterministic masternode manager\n");
                
                // Register validation interface to receive block updates
                RegisterValidationInterface(deterministicMNManager.get());
            }
            
            // ============================================================
            // LLMQ Subsystem Initialization
            // Long-Living Masternode Quorums for InstantSend and ChainLocks
            // ============================================================
            {
                // Initialize quorum manager and signing manager
                llmq::InitLLMQ();
                LogPrintf("Initialized LLMQ core subsystem\n");
                
                // Initialize masternode identity if configured
                // This must be done after deterministicMNManager is ready
                if (gArgs.GetBoolArg("-masternode", false)) {
                    std::string operatorKeyHex = gArgs.GetArg("-masternodeblsprivkey", "");
                    if (operatorKeyHex.empty()) {
                        return InitError("Masternode mode enabled but -masternodeblsprivkey not set");
                    }
                    
                    // Validate key format (64 hex chars = 32 bytes)
                    if (operatorKeyHex.length() != 64 || !IsHex(operatorKeyHex)) {
                        return InitError("Invalid -masternodeblsprivkey format. Must be 64 hex characters.");
                    }
                    
                    // Store the key for deferred initialization
                    // The actual identity initialization happens after the blockchain
                    // is synced and we have the masternode list available.
                    // SetPendingMasternodeKey stores the key, and TryInitMasternodeIdentity
                    // is called from UpdatedBlockTip to complete initialization.
                    llmq::SetPendingMasternodeKey(operatorKeyHex);
                    LogPrintf("Masternode mode enabled, identity will be initialized after sync\n");
                }
                
                // Initialize InstantSend (requires LLMQ)
                if (llmq::signingManager && llmq::quorumManager) {
                    llmq::InitInstantSend(*llmq::signingManager, *llmq::quorumManager);
                    LogPrintf("Initialized InstantSend subsystem\n");
                }
                
                // Initialize ChainLocks (requires LLMQ and evoDb for persistence)
                if (llmq::signingManager && llmq::quorumManager && evoDb) {
                    llmq::InitChainLocks(*evoDb, *llmq::signingManager, *llmq::quorumManager);
                    LogPrintf("Initialized ChainLocks subsystem with persistence\n");
                }
                
                // Initialize PoSe (Proof of Service) manager
                if (evoDb) {
                    InitPoSe(*evoDb);
                }
            }
