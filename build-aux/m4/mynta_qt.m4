...
dnl Output: If qt version is auto, set mynta_enable_qt to false. Else, exit.
AC_DEFUN([MYNTA_QT_FAIL],[
  if test "x$mynta_qt_want_version" = xauto && test "x$mynta_qt_force" != xyes; then
    if test "x$mynta_enable_qt" != xno; then
      AC_MSG_WARN([$1; raven-qt frontend will not be built])
    fi
    mynta_enable_qt=no
    mynta_enable_qt_test=no
  else
    AC_MSG_ERROR([$1])
  fi
])
...
AC_DEFUN([MYNTA_QT_CHECK],[
  if test "x$mynta_enable_qt" != xno && test "x$mynta_qt_want_version" != xno; then
    true
    $1
  else
    true
...
AC_DEFUN([MYNTA_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [
     mynta_qt_want_version=$withval
     if test "x$mynta_qt_want_version" = xyes; then
       mynta_qt_force=yes
       mynta_qt_want_version=auto
     fi
    ],
    [mynta_qt_want_version=auto])
  ...
  AS_IF([test "x$with_gui" = xqt5_debug],
        [AS_CASE([$host],
                 ...
                 [qt_lib_suffix= ]); mynta_qt_want_version=qt5],
        [qt_lib_suffix= ])
...
dnl Outputs: mynta_enable_qt, mynta_enable_qt_dbus, mynta_enable_qt_test
AC_DEFUN([MYNTA_QT_CONFIGURE],[
  ...
  _MYNTA_QT_IS_STATIC
  if test "x$mynta_cv_static_qt" = xyes; then
  ...
  MYNTA_QT_CHECK([
    mynta_enable_qt=yes
    mynta_enable_qt_test=yes
    if test "x$have_qt_test" = xno; then
      mynta_enable_qt_test=no
    fi
    mynta_enable_qt_dbus=no
    if test "x$use_dbus" != xno && test "x$have_qt_dbus" = xyes; then
      mynta_enable_qt_dbus=yes
    fi
  ...
  if test x$mynta_enable_qt = xyes; then
    AC_MSG_RESULT([$mynta_enable_qt ($qt_lib_prefix)])
  else
    AC_MSG_RESULT([$mynta_enable_qt])
  fi
...
dnl Output: mynta_cv_static_qt=yes|no
AC_DEFUN([_MYNTA_QT_IS_STATIC],[
  AC_CACHE_CHECK(for static Qt, mynta_cv_static_qt,[
    AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
        #include <QtCore/qconfig.h>
        #ifndef QT_VERSION
        #  include <QtCore/qglobal.h>
      ...
      [mynta_cv_static_qt=yes],
      [mynta_cv_static_qt=no])
    ])
])
